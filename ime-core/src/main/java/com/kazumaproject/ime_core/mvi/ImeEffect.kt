...
sealed interface ImeEffect {
    ...
    data class RenderPreedit(val composing: String, val cursor: Int, val decor: PreeditDecor) :
        ImeEffect
    ...
    // ★候補要求（副作用）
    data class RequestCandidates(val requestKey: String, val bgText: String, val limit: Int = 8) :
        ImeEffect
}
